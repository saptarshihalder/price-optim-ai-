...
const CompetitiveAnalysis: React.FC = () => {
  const [scrapingResults, setScrapingResults] = useState<ScrapedProduct[]>([]);
  ...
  const loadLatestScrapingResults = async () => {
    try {
      setLoading(true);
      // For demo, we'll use the results from our test scraping
      // In real implementation, you'd get the latest task_id from local storage or API
      const taskId = 'scrape_1754930923';
      
      const response = await brain.get_scraping_results({ taskId: taskId });
